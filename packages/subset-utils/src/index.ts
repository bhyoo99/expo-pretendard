...
const FONTWEIGHTS = ["Black", "Bold", "ExtraBold", "ExtraLight", "Light", "Medium", "Regular", "SemiBold", "Thin"];
...
export function getFontList(family = FONTFAMILY.Pretendard, options?: TFontListOptions) {
    ...
    const extResult = `.${ext}`;
    const fontList = variable ? [family + "Variable" + extResult] : FONTWEIGHTS.map((weight) => family + "/" + weight + extResult);
